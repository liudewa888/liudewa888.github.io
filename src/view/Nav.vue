<template>
  <el-row justify="center">
    <!-- 搜索栏start -->
    <div class="search">
      <el-input v-model="searchVal" clearable input-style="border:none;" @keyup.enter="gotoSearch">
        <template #prepend>
          <el-select v-model="searchEngine" style="width: 90px" size="large" @change="searchEngineChange">
            <el-option :label="item.name" :value="index" v-for="(item, index) of searchs" />
          </el-select>
        </template>
        <template #append>
      ...
      <div class="jj-list" v-for="item of navigationList">
        ...
        <ul class="jj-list-con">
          <li v-for="item1 of item.children">
            <a :href="item1.link" class="link-3" rel="nofollow" target="_blank" @click="jumpA(item1.link, $event)">{{
              item1.title
            }}</a>
          </li>
        </ul>
    ...
    <el-dialog v-model="dialogShow" title="高级搜索" :show-close="false" :close-on-click-modal="false" @close="dialogClose">
      <el-form>
        <el-form-item label="来源 : ">
          <el-space :size="1">
            <el-checkbox v-model="checkAllIncludes" :indeterminate="isIndeterminateIncludes"
              @change="handleCheckAllIncludesChange">全选</el-checkbox>
          </el-space>
          <el-checkbox-group v-model="checkedIncludes" @change="handleCheckedIncludesChange">
            <el-checkbox v-for="item in includes" :key="item.link" :label="item.link">{{
              item.title
            }}</el-checkbox>
          </el-checkbox-group>
        </el-form-item>
        <el-form-item label="排除 : ">
          <el-space :size="1">
            <el-checkbox v-model="checkAllExcludes" :indeterminate="isIndeterminateExcludes"
              @change="handleCheckAllExcludesChange">全选</el-checkbox>
          </el-space>
          <el-checkbox-group v-model="checkedExcludes" @change="handleCheckedExcludesChange">
            <el-checkbox v-for="item in excludes" :key="item.link" :label="item.link">{{
              item.title
            }}</el-checkbox>
          </el-checkbox-group>
        </el-form-item>
      ...
      <template #footer>
        <span class="dialog-footer">
          ...
          <el-button type="primary" @click="saveStorage">
            保存
          </el-button>
        </span>
      </template>
...
const saveStorage = () => {
  let temp = null
  if (checkedIncludes.value.length) {
    temp = {
      index: '0',
      checkedList: checkedIncludes.value
    }
  }
  if (checkedExcludes.value.length) {
    temp = {
      index: '1',
...
const init = () => {
  ...
  if (storage) {
    if (storage.index === '0') {
      ...
      checkAllIncludes.value = checkedCount === includes.length
      isIndeterminateIncludes.value = checkedCount > 0 && checkedCount < excludes.length
    }
    if (storage.index === '1') {
      const checkedCount = storage.checkedList.length
